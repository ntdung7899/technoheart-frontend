...
export default async function ProductDetailPage({ params }: Props) {
  ...
  const mainImage = product.imageUrl || images[0] || "/placeholder.png";
      ...
      <div className="container mx-auto px-4 py-4 lg:px-8 max-w-[1200px]">
        ...
        <div className="mb-6 flex items-center justify-between">
          ...
          >
            ...
            Quay lại {categoryName.toLowerCase()}
          </Link>
              ...
              {images.length > 1 && (
                <div className="grid grid-cols-4 gap-2.5">
                  {images.map((img: string, idx: number) => (
                    <button
                      key={idx}
                      className={`relative aspect-square overflow-hidden rounded-xl border-2 bg-white p-1.5 transition-all hover:border-primary/60 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-primary/20 ${
                        idx === 0
            ...
            <div className="flex flex-col">
              ...
              <div className="mb-3">
                <span className="inline-flex items-center gap-1.5 text-sm text-zinc-500">
                  <span className="font-bold text-zinc-800 bg-zinc-100 px-2.5 py-0.5 rounded-md text-xs">
                    {categoryName}
                  </span>
              ...
              <div className="space-y-3.5 mb-8">
                <div className="flex items-center gap-3 text-sm text-zinc-600">
                  ...
                  <span>{product.warranty || "Bảo hành theo chính sách sản phẩm"}</span>
                </div>

                <div className="flex items-center gap-3 text-sm text-zinc-600">
                  <Truck className="h-5 w-5 text-primary flex-shrink-0" />
                  <span>{product.shippingInfo || "Hỗ trợ giao hàng toàn quốc"}</span>
                </div>

                <div className="flex items-center gap-3 text-sm text-zinc-600">
                  <RefreshCw className="h-5 w-5 text-primary flex-shrink-0" />
                  <span>{product.returnPolicy || "Đổi trả theo chính sách"}</span>
                </div>
